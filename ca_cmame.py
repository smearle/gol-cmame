...
class NNGoL(torch.nn.Module):
    def __init__(self):
        self.m = 3
        super().__init__()
        self.embed = Conv2d(3, 3, 1, 1, 0, bias=True)
        self.embed.to('cuda:0')
        self.l1 = Conv2d(3, 3 * self.m, 3, 1, 1, bias=True, padding_mode='circular')
        self.l2 = Conv2d(3 * self.m, self.m, 1, 1, 0, bias=True)
        self.l3 = Conv2d(self.m, 3, 1, 1, 0, bias=True)
