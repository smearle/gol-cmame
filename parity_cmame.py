...
class Parity(gym.core.Env):
    ...
    def step(self, actions):
        ...
        self.label = (np.sum(self.input, axis=1) % 2 == 1).astype(int).reshape(
            -1, 1)
    ...
    def render(self, mode=None):
        if not hasattr(self, 'label'):
            return
        print('Inputs: {}\nLabel: {}\nOutput: {}'.format(
            self.input, self.label, self.actions))
...
class NNParity(torch.nn.Module):
    def __init__(self):
        ...
        for i in range(int(np.log2(n_inputs))):
            self.layers.append(
                torch.nn.Linear(int(n_inputs / 2**i), int(n_inputs / 2**i)))
            self.layers.append(
                torch.nn.Linear(int(n_inputs / 2**i),
                                int(n_inputs / 2**(i + 1))))
        [
            setattr(self, 'l{}'.format(i), l)
    ...
    def perturb_weights(self, scale):
        ...
        for l in self.layers:
            l.weight.data = l.weight.data.cpu() + np.random.normal(
                0, scale, l.weight.data.shape)
            l.bias.data = l.bias.data.cpu() + np.random.normal(
                0, scale, l.bias.data.shape)
...
def simulate(env, nn, model, seed=None, state=None):
    ...
    while not done:
        ...
        action = nn(obs)
        if ME:
            bcs_0.append(action.mean().item())
            bcs_1.append(action.std().item())
    ...
    if ME:
        bc = (np.mean(bcs_0), np.mean(bcs_1))
    else:
        bc = (0)
    #   print(bc)
        ...
        for itr in tqdm.tqdm(range(1, total_itrs + 1)):
            ...
            for (i, model) in enumerate(sols):
                ...
                for j in range(self.n_sims):
                    ...
                    m_bcs.append(bc)
                    m_objs.append(obj)
                bc = (np.mean([b[0] for b in m_bcs]), np.mean([b[1] for b in m_bcs]))
                bcs[i, :] = bc
...
if __name__ == '__main__':
    ...
    opts.add_argument('-s', '--size', help='Size of world grid', default=700)
    opts.add_argument('-p',
                      '--prob',
                      help='Probability of life in the initial seed',
                      default=.05)
    opts.add_argument('-tr',
                      '--tick_ratio',
    ...
    opts.add_argument('-e', '--exp_name', default='parity_0')
    opts.add_argument(
        '--n_inputs',
        default=16,
    )
    ...
    opts.add_argument('--perturb', default=0.1)
    ...
    global N_INPUTS
    global TRAIN_BRUTE
    global ME  # are we doing CMAME? Otherwise just CMAES
    ME = opts.ME
    if not ME:
        algo_name = 'cmaes'
    else:
    ...
    SAVE_PATH = os.path.join(
        'experiments', 'parity_n_in_{}_randInit_algo_{}_{}'.format(N_INPUTS, algo_name, opts.exp_name))
